...
## Punto E
Permite que un jugador ingrese un número de 4 dígitos no repetidos y permite que otro jugador ingrese posibles jugadas, hasta cuando adivine el número secreto. El programa finalizará cuando se adivine el número o cuando el usuario no desee continuar. El algoritmo deberá mostrar la cantidad de jugadas realizadas.

1. Declaro `numSecreto[]` y `numJugada[]`
1. Declaro `pica` y `fija`
1. Solicito número secreto
1. Llamo función `numerosNorepetidos`
1. Mientras `fijas`<tam
    1. Solicito jugadas
    1. Llamo función `numerosNorepetidos`
    1. Llamo función `picas`
    1. Llamo función `fijas`
    1. Incremento `jugada` en 1
    1. Confirmar continuar juego 
    1. Asigno valor a `a`
        1. Si `a` = 0
        1. Finalizo juego e imprimo jugadas 
1. Finalizo juego e imprimo jugadas
